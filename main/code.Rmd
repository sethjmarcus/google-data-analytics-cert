...
```{r}
# removing trips longer than 2 days 
year_2022_v3 = year_2022_v2[!(year_2022_v2$ride_length > 172800),]
```
...
```{r}
summary(as.numeric(year_2022_v3$ride_length))
```
...
year_2022_v2 %>% 
  ...
  group_by(member_casual, weekday) %>% 
  summarise(number_of_rides = n(),
            manhattan_distance = abs(start_lat-end_lat) + abs(start_lng-end_lng)) %>% 
  arrange(member_casual, weekday) %>% 
